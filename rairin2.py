...
import urllib3
import requests
from io import BytesIO  # PENTING: Untuk memproses gambar di RAM
...
# TEMP_DIR dihapus karena kita pakai RAM (BytesIO)
...
async def fetch_master_source(specific_tags=None):
    ...
    # Wrapper untuk Parallel Request
    async def fetch_source(src):
        try:
            params = {"tags": query, "limit": 20}
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, list):
                    return parse_general_results(data, src['name'])
        except: 
            return []
        return []
    ...
    if len(candidates) < 3 or (not specific_tags and not candidates):
        try:
...
# ==========================================
# 2. DOWNLOAD & SEND (OPTIMIZED - RAM ONLY)
# ==========================================
...
async def smart_send_photo(update, image_url, caption, loading_msg=None):
    try:
        # Update Loading Text
        if loading_msg: 
            try: await loading_msg.edit_text("💕 <i>Hunting Bini...</i>", parse_mode=ParseMode.HTML)
            except: pass

        # Download langsung ke RAM menggunakan CloudScraper
        loop = asyncio.get_running_loop()
        def download_to_ram():
            # Stream=True, timeout cepat
            resp = scraper.get(image_url, stream=True, timeout=15)
            resp.raise_for_status()
            return BytesIO(resp.content)
        
        image_data = await loop.run_in_executor(None, download_to_ram)
        image_data.name = "image.jpg" # Telegram perlu nama dummy
        ...
        # Kirim Gambar dari RAM
        try:
            await update.message.reply_photo(photo=image_data, caption=caption, parse_mode=ParseMode.HTML)
        except BadRequest:
            # Fallback kirim file jika gagal render foto
            image_data.seek(0)
...
async def admin_system_control(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    shutdown_keywords = ["shutdown", "Rairin shutdown system", "shutdown system", "terminate", "suspend"]
...
async def swing_waifu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    PENDING_TRADES[trade_id] = {
        ...
        "p2": target_owner_id, "p2_name": target_name, "c2": target_char
    }

    kb = [[InlineKeyboardButton("✅ ACCEPT TRADE", callback_data=f"swing_ok_{trade_id}"), InlineKeyboardButton("❌ REJECT", callback_data=f"swing_no_{trade_id}")]]
    msg_txt = (
