...
def parse_general_results(posts, source_name):
    ...
    for post in posts:
        ...
        if char_tags: name = char_tags[0].replace('_', ' ').title()
        else:
...
async def fetch_master_source(specific_tags=None):
    ...
    async def fetch_source(src):
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                if src['type'] == 'gelbooru' and isinstance(data, dict) and 'post' in data: data = data['post']
                if isinstance(data, list): return parse_general_results(data, src['name'])
        except: return []
        return []
    ...
    if len(candidates) < 3 or (not specific_tags and not candidates):
        try:
            ...
            if resp.status_code == 200:
                data = resp.json()
                if 'images' in data: candidates.extend(parse_waifu_results(data['images'], w_tag))
        except: pass
